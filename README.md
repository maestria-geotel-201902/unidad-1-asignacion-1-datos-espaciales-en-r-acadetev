...
## Reading layer `PROVCenso2010' from data source `/home/acade/unidad-1-asignacion-1-datos-espaciales-en-r-acadetev/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
plot(prov[prov$PROV %in% '11', 'PROV'])
```
...
## 11   11  08 LA ALTAGRACIA   0811 MULTIPOLYGON (((516555.9 20...
plot(miprov)
```
...
##           areaenm2
## 1 2996054190 [m^2]
prov %>% dplyr::filter(PROV %in% '11') %>% dplyr::select(areaenm2) %>% plot
```
...
## Reading layer `MUNCenso2010' from data source `/home/acade/unidad-1-asignacion-1-datos-espaciales-en-r-acadetev/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
mimun <- mun %>% dplyr::filter(PROV %in% '11')
mimun %>% plot
```
